...
func TestCompletionReplayRejectsChangedManureWeightWithoutDuplicatingWrites(t *testing.T) {
	db := bootstrapDatabase(t)
	scheduledPlan(t, db, "plan-replay-contract", 100)
	ctx := context.Background()
	repo := FeedRepository{DB: db}
	input := completionInput("plan-replay-contract", "replay-contract")
	originalRound, originalBatch, err := repo.Complete(ctx, input)
	if err != nil {
		t.Fatalf("first completion: %v", err)
	}

	changed := input
	changed.RoundID = "unused-round"
	changed.BatchID = "unused-batch"
	changed.RequestID = "unused-request"
	changed.AuditID = "unused-audit"
	changed.OutboxID = "unused-job"
	changed.ManureKg = input.ManureKg + 12
	if _, _, err := repo.Complete(ctx, changed); !errors.Is(err, domain.ErrConflict) {
		t.Fatalf("changed manure weight replay error = %v, want conflict", err)
	}

	replayedRound, replayedBatch, err := repo.Complete(ctx, input)
	if err != nil {
		t.Fatalf("identical replay: %v", err)
	}
	if replayedRound.ID != originalRound.ID || replayedBatch.ID != originalBatch.ID {
		t.Fatalf("identical replay returned round=%s batch=%s, want round=%s batch=%s", replayedRound.ID, replayedBatch.ID, originalRound.ID, originalBatch.ID)
	}

	var rounds, batches, audits, jobs int
	if err := db.Pool.QueryRow(ctx, "SELECT count(*) FROM feed_rounds WHERE plan_id=$1", input.PlanID).Scan(&rounds); err != nil {
		t.Fatal(err)
	}
	if err := db.Pool.QueryRow(ctx, "SELECT count(*) FROM manure_batches WHERE source_round_id=$1", originalRound.ID).Scan(&batches); err != nil {
		t.Fatal(err)
	}
	if err := db.Pool.QueryRow(ctx, "SELECT count(*) FROM audit_events WHERE object_id=$1 AND action='feed.complete'", input.PlanID).Scan(&audits); err != nil {
		t.Fatal(err)
	}
	if err := db.Pool.QueryRow(ctx, "SELECT count(*) FROM outbox_jobs WHERE topic='manure.batch.collected' AND payload->>'plan_id'=$1", input.PlanID).Scan(&jobs); err != nil {
		t.Fatal(err)
	}
	if rounds != 1 || batches != 1 || audits != 1 || jobs != 1 {
		t.Fatalf("replay duplicated writes: rounds=%d batches=%d audits=%d jobs=%d", rounds, batches, audits, jobs)
	}
}
