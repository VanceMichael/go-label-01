...
type storedFeedCompletion struct {
	round domain.FeedRound
	batch domain.ManureBatch
}

func findStoredFeedCompletion(ctx context.Context, tx pgx.Tx, in feed.CompleteInput) (storedFeedCompletion, bool, error) {
	var stored storedFeedCompletion
	err := tx.QueryRow(ctx, `
		SELECT id, tenant_id, plan_id, idempotency_key, status, delivered_kg, recorded_at
		FROM feed_rounds
		WHERE tenant_id=$1 AND idempotency_key=$2
	`, in.TenantID, in.IdempotencyKey).Scan(
		&stored.round.ID,
		&stored.round.TenantID,
		&stored.round.PlanID,
		&stored.round.IdempotencyKey,
		&stored.round.Status,
		&stored.round.DeliveredKg,
		&stored.round.RecordedAt,
	)
	if errors.Is(err, pgx.ErrNoRows) {
		return storedFeedCompletion{}, false, nil
	}
	if err != nil {
		return storedFeedCompletion{}, false, fmt.Errorf("find feed completion replay: %w", err)
	}

	err = tx.QueryRow(ctx, `
		SELECT id, tenant_id, group_id, source_round_id, status, weight_kg, collected_at, version
		FROM manure_batches
		WHERE tenant_id=$1 AND source_round_id=$2
	`, in.TenantID, stored.round.ID).Scan(
		&stored.batch.ID,
		&stored.batch.TenantID,
		&stored.batch.GroupID,
		&stored.batch.SourceRoundID,
		&stored.batch.Status,
		&stored.batch.WeightKg,
		&stored.batch.CollectedAt,
		&stored.batch.Version,
	)
	if err != nil {
		return storedFeedCompletion{}, false, fmt.Errorf("load manure batch for replay: %w", err)
	}
	return stored, true, nil
}

func validateFeedCompletionReplay(stored storedFeedCompletion, in feed.CompleteInput) error {
	if stored.round.PlanID != in.PlanID {
		return fmt.Errorf("%w: idempotency key belongs to another feed plan", domain.ErrConflict)
	}
	if stored.round.DeliveredKg != in.DeliveredKg {
		return fmt.Errorf("%w: idempotency key reused with different delivered weight", domain.ErrConflict)
	}
	return nil
}

func resolveStoredFeedCompletion(ctx context.Context, tx pgx.Tx, in feed.CompleteInput) (domain.FeedRound, domain.ManureBatch, bool, error) {
	stored, found, err := findStoredFeedCompletion(ctx, tx, in)
	if err != nil {
		return domain.FeedRound{}, domain.ManureBatch{}, false, err
	}
	if !found {
		return domain.FeedRound{}, domain.ManureBatch{}, false, nil
	}
	if err := validateFeedCompletionReplay(stored, in); err != nil {
		return domain.FeedRound{}, domain.ManureBatch{}, true, err
	}
	return stored.round, stored.batch, true, nil
}
...
func (r FeedRepository) Complete(ctx context.Context, in feed.CompleteInput) (domain.FeedRound, domain.ManureBatch, error) {
	...
	err := r.DB.WithTx(ctx, func(tx pgx.Tx) error {
		replayedRound, replayedBatch, replayed, err := resolveStoredFeedCompletion(ctx, tx, in)
		if err != nil {
			return err
		}
		if replayed {
